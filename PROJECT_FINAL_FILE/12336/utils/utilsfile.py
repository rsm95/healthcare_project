...
class ClaimsProcessing:
    ...
    def load_data(self):
        try:
            self.df = self.spark.read.csv(self.file_path, header=True, schema=self.schema)
    ...
    def identify_outliers(self, mean_claim, stddev_claim):
        try:
            ...
            outliers_2sigma = self.df.filter((col("claim_amount_settled") > threshold_high_2sigma))
            outliers_3sigma = self.df.filter((col("claim_amount_settled") > threshold_high_3sigma))
    ...
    def identify_frequent_and_fraudulent_claims(self, disease, threshold_high_2sigma):
        try:
            ...
            fraudulent_claims = fraudulent_claims.join(frequent_claims, on="hospital_id", how="left")
